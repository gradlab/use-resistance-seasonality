...
#Make Figure 1a (raw antibiotic use)
labels = data.frame(drug_class = c("Macrolides", "Nitrofurans", "Penicillins", "Quinolones", "Tetracyclines"),
                    x.pos = c(4, 4, 4, 4, 4),
...
#Make figure 1b (seasonal antibiotic use models)
#Cosine function
...
plot_use_model_func = function(deviates, class, amplitude, phase, omega, a_lower, a_upper) {
...
#Make individual plots for figure 1b
f1b_plots = use.model.params %>%
  ...
  mutate(plot = pmap(.l = list(deviates = deviates_table, class = drug_class, amplitude = amplitude_estimate,
                               ...
                               a_upper = amplitude_ci.upper),
                     .f = plot_use_model_func)) %>%
  pull(plot)

#Combine plots to make figure 1b
f1b = do.call(plot_grid, c(f1b_plots, nrow = 3, ncol = 2, align = "hv")) %>%
  annotate_figure(left = text_grob("Seasonal deviates in use (claims/1000 people)", size = 11, rot = 90)) 
...
plot_use_resistance_func = function(deviates, drug, class, u_a, u_p, u_o, u_low, u_up, r_a, r_p, r_o, r_low, r_up, ratio) {
  col = colors[class]
...
f2 = do.call(plot_grid, c(f2_plots, nrow = 2, ncol = 3)) %>%
  annotate_figure(left = text_grob("Seasonal deviates in resistance (log2(MIC))", size = 11, rot = 90)) %>%
  annotate_figure(right = text_grob("Seasonal deviates in use (claims/1000 people)", size = 11, rot = 270)) %>%
...
plot_amplitudes_func = function(dat, title) {
  p = ggplot(data = dat, aes(x = reorder(drug_code, estimate), y = estimate, color = drug_class)) +
    ...
    scale_y_continuous(breaks = c(0, 0.05, 0.1), limits = c(-0.01, 0.1)) +
...
f4 = ggplot(data = res.phases) +
  ...
  scale_y_continuous(breaks = seq(-2 ,11, 1), limits = c(-2.2, 11),
...
plot_corr_func = function(dat, o) {
  ...
  p1 = ggplot(data = d, aes(x = reorder(use_res, rho), y = rho)) +
    geom_point() +
  ...
  p2 = ggplot(data = d, aes(x = reorder(use_res, rho), y = 1, fill = use_class)) +
  ...
  p = plot_grid(p2, p1, nrow = 1, ncol = 2, rel_widths = c(1, 1.5), align = "h") %>%
    annotate_figure(top = text_grob(o, face = "bold.italic", size = 12))
  
  return(p)
}

f5_SA = plot_corr_func(correlations, "S. aureus")
f5_EC = plot_corr_func(correlations, "E. coli")
f5_KP = plot_corr_func(correlations, "K. pneumoniae")

#Make figure 5
f5 = plot_grid(f5_SA, f5_EC, f5_KP, ncol = 3, nrow = 1)

# ######################################################
# Save all figures
# ######################################################

ggsave(f1, filename = "figures/Fig1.pdf", width = 7.5, height = 5.5)
ggsave(f2, filename = "figures/Fig2.pdf", width = 7.5, height = 5.5)
ggsave(f3, filename = "figures/Fig3.pdf", width = 6.5, height = 4)
ggsave(f4, filename = "figures/Fig4.pdf", width = 6, height = 5.5)
ggsave(f5, filename = "figures/Fig5.pdf", width = 7.5, height = 6)

ggsave(fS1, filename = "figures/FigS1.pdf", width = 7.5, height = 5.5)
ggsave(fS2, filename = "figures/FigS2.pdf", width = 5, height = 5.5)
ggsave(fS3, filename = "figures/FigS3.pdf", width = 5, height = 2.75)
