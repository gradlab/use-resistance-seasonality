...
#to a sinusoidal model with a linear correction and adjustment for age and sex (Model B)
...
#Run resistance seasonality regressions for each org/drug with 6 and 12m periods using Model B
results = data.res %>%
  ...
  #run the regression
  mutate(model = pmap(.l = list(data = data, model_matrix = model_matrix, omega = omega, on="hos_year"), .f = model_B_func)) %>%
  #calculate the AIC for each model
  mutate(AIC = map_dbl(model, ~ AIC(.))) %>%
  #extract model parameters into a table
  mutate(model_summary = pmap(.l = list(model = model, model_matrix = model_matrix, on = "hos_year", num_covariates = 2), .f = get_model_summary_func)) 
...
write_csv(model.params.full, "tables/resistance_modelB_values.csv")
write_csv(deviates.table, "tables/resistance_modelB_seasonal_deviates.csv")
