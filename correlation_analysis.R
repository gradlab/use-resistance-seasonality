...
deviates.use = read_csv("data/seasonal_deviates_use.csv")
deviates.res = read_csv("data/seasonal_deviates_resistance.csv")
...
use.res.deviates = left_join(
  deviates.res %>%
    ...
    select(org, month, drug_code, drug_name, drug_class, res = seasonal_deviate),
...
correlations = use.res.deviates %>%
  nest(-org, -drug_code, -drug_name, -drug_class, -use_class) %>%
  ...
  select(org, res_drug_code = drug_code, res_drug_name = drug_name, res_drug_class = drug_class, use_class,
